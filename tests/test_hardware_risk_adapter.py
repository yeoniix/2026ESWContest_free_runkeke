...
def _run_baseline(adapter, *, ticks: int = 30, **packet_kwargs):
    ...
    for sequence in range(ticks):
        reading = adapter.update(
            _packet(sequence, flags=0b1001, **packet_kwargs), sequence * 1_000
        )
    return reading
...
def test_risk_score_is_unchanged_by_the_weight_redistribution():
    ...
    design_reading = design.update(_packet(30, flags=0b1001, **hot), 30_000)
    hardware_reading = hardware.update(_packet(30, flags=0b1001, **hot), 30_000)
