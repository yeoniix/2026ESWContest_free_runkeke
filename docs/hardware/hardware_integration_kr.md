...
| 3 | 1 | `state` | `StateCode` |
| 4 | 1 | `cause` | `CauseCode` |
| 5 | 1 | `fanPercent` | 두 팬 채널의 최댓값. 현재 0 또는 100 |
| 6 | 1 | `bpm` | 0~255 |
...
`makeDisplayStatus()`가 0.5초마다 호출되며, 위에서 먼저 만족한 조건이 최종 상태가 된다.

| 우선 | 조건 | 결과 상태 | 원인 |
| ---: | --- | --- | --- |
| 1 | 비상 버튼 상태 활성 | `EMERGENCY` | `NONE` |
| 2 | 손목 데이터 없음/10초 초과, Finger NO, BPM ≤ 0 | `SENSOR_CHECK` | `SENSOR` (기준선 초기화) |
| 3 | 기준선 수집 3분 미만 | `BASELINE` | `NONE` |
| 4 | 아래 위험점수 표에 따름 | — | `determineCause()` |

#### 벨트 자체 위험점수 `calculateRisk()`

RiskIndex v0.3을 포팅한 것이 아니라, 벨트가 통신 두절 상태에서도 단독 판단할 수 있도록 만든
**구간별 가산식**이다. 유효 표본이 10개 미만이면 0을 반환한다.

| 항목 | 조건 | 가산 |
| --- | --- | ---: |
| BPM 편차 | 기준선 대비 `≥ 40` | +60 |
| | 기준선 대비 `≥ 25` | +40 |
| 피부온도 | 편차 `≥ 0.9°C` 또는 상승률 `≥ 0.20°C/분` | +55 |
| | 편차 `≥ 0.5°C` 또는 상승률 `≥ 0.10°C/분` | +35 |
| GSR | 절대 편차 `≥ max(10, 기준선/20)` 또는 상대 변화 `≥ 15%` | +25 |

합계를 0~100으로 clamp한다. `risk ≥ 60`이 **10초 연속** 유지되면 `held10s`가 성립한다.

| 위험점수 | `held10s` 미성립 | `held10s` 성립 |
| --- | --- | --- |
| `< 40` | `NORMAL` | `NORMAL` |
| `40 ~ 59` | `CAUTION` | `CAUTION` |
| `60 ~ 84` | `CAUTION` | `COOLING_50` |
| `≥ 85` | `CAUTION` | `DANGER` |

원인 코드는 `determineCause()`가 BPM 편차 `≥ 25` → `HR_HIGH`, 피부온도 편차 `≥ 0.5` → `TEMP_UP`,
GSR 편차 초과 → `GSR_UP` 순서로 하나만 고른다.

> 게이트웨이의 RiskIndex v0.3과는 입력 특징도 임계값도 다르다. 두 판정을 하나로 합치지 않고
> 각각 보존하는 이유와 대응표는 [../api_contract.md](../api_contract.md)의 "판정이 두 벌인 이유" 참고.

### 9.3 팬 제어

두 팬을 개별 PWM 채널로 제어한다(`setFanPins(pct1, pct2)`). 냉각 1단계는 듀티를 낮추는 대신
**팬 하나만 돌리는** 방식이다 — 저출력 듀티는 체감 냉각 효과가 작기 때문이다.

| 조건 | FAN1 | FAN2 |
| --- | --- | --- |
| 비상 활성 | 100% | 100% |
| 손목 데이터 없음/10초 초과, Finger NO, BPM ≤ 0 | OFF | OFF |
| 기준선 수집 중 | OFF | OFF |
| `NORMAL` / `CAUTION` (risk < 60 또는 10초 미유지) | OFF | OFF |
| `COOLING_50` (risk 60~84, 10초 유지) | **100%** | OFF |
| `DANGER` (risk ≥ 85, 10초 유지) | 100% | 100% |

> **이름 주의:** `STATE_COOLING_50`의 `50`은 PWM 듀티 50%가 아니라 **팬 2개 중 1개**를 뜻한다.
> `DisplayPacket.fanPercent`는 두 채널의 최댓값이므로 이 단계에서도 `100`이 전달된다. 따라서
> 관제의 `cooling.actual_pwm`만으로는 1단계와 2단계를 구분할 수 없고, `raw.belt_state`를 함께
> 봐야 한다.
